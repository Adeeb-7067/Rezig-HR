...
const Level0MenuItem = ({ item, active, onClick }) => {
  return (
    <div
      onClick={onClick}
      className={`flex items-center gap-2 p-2 text-[0.75rem] font-semibold rounded-sm cursor-pointer
      ${active
          ? "bg-[#8629DF] text-white"
          : "text-gray-800 hover:bg-[#8629DF] hover:text-white"
        }`}
    >
      {item.icon}
      {item.name}
...
const Level1MenuItem = ({ item, active, onClick }) => {
  return (
    <div className="ml-4 relative">
      <div
        onClick={onClick}
        className={`flex items-center text-[0.75rem] font-semibold px-2 py-1 cursor-pointer rounded-sm
        ${active
      ...
      >
        {active && (
          <GoTriangleRight className="absolute -left-4 text-[#8629DF]" />
        )}
                ...
                <div className="mt-1 space-y-1">
                  {item.children.map((child) => (
                    <Level1MenuItem
                      ...
                      onClick={() =>
                        handleNavigation(child.name, child.path)
                      }
                    />
                  ))}
                </div>
